...
| `src/lib/runs/stateMachine.ts` | Deterministic run lifecycle orchestrator. Drives `ingested → extracting → awaiting_human → filling → zipping → done`. Validates transitions, publishes `run.<status>` events, spawns Copilot for extraction/filling, performs post-fill filename normalization. Uses multi-strategy `resolveExtractedJson()` to handle LLM non-determinism: tries finalAssistantText, then disk output.json, then tool result payloads. Calls `sanitizeExtractedJson()` before Zod validation to strip `null` / `""` / `"N/A"` / `"Unknown"` / `"None"` and similar sentinel strings that the model emits for missing optional fields, converting them into omitted keys so a single sloppy field cannot torpedo an otherwise-correct extraction. | `transition()`, `failRun()`, `advanceRun()`, `resumeAfterApproval()`, `extractJsonFromText()`, `resolveExtractedJson()`, `sanitizeExtractedJson()`, `runDir()` | `runs/route.ts`, `approve/route.ts` |
...
| `tests/unit/stateMachine.test.ts` | Unit | `src/lib/runs/stateMachine.ts` | Legal/illegal transition validation, `failRun` no-op on terminal states, `extractJsonFromText` direct parse / markdown fence / brace extraction, `resolveExtractedJson` multi-strategy fallback (message → disk → tool results), `sanitizeExtractedJson` null/empty/`"N/A"`/`"Unknown"`/`"None"` sentinel stripping at top-level + owner objects, non-object owner pruning, end-to-end null-laden LLM output passing Zod after sanitization |
